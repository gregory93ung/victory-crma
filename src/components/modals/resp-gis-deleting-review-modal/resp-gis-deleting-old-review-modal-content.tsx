...
import { addRespDeletingOldReviewGisReq } from '@/api/gis/addRespDeletingOldReviewGis';

interface RespGisDeletingOldReviewModalContentProps {
...
export const formSchema = z.object({
  claim_text: z
    ...
    .min(10, {
      message: 'Для отправки жалобы необходимо минимум 10 символов',
    }),
});

export function RespGisDeletingOldReviewModalContent({
  reviewInfo,
  firmUrl,
  orderId,
  serviceId,
  setOpen,
}: RespGisDeletingOldReviewModalContentProps) {
  const form = useForm<z.infer<typeof formSchema>>({
    resolver: zodResolver(formSchema),
  });

  const onRespDeletingOldReviewGis = (data: any) => {
    toast('Добавление жалобы на отзыв', {
      description: data.detail,
    });
    form.reset();
    queryClient.invalidateQueries(['getDeletingOldReviewsGis']);
    setOpen(false);
  };

  const { mutate, isLoading } = useMutation(
    clients.addRespDeletingOldReviewGis.fn,
    {
      onSuccess: (data) => onRespDeletingOldReviewGis(data),
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    mutate({
      body: {
        ...
        ...cleanedData,
      } as addRespDeletingOldReviewGisReq,
    });
    setOpen(false);
  }
