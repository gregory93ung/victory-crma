...
import { addRespAccountingReviewYandexReq } from '@/api/yandex/addRespAccountingReviewYandex';
import { AxiosError } from 'axios';

interface resYandexAccountingReviewModalProps {
  reviewInfo: Clients.YandexAccountingReview;
  firmUrl: string;
  orderId: number;
  serviceId: number;
  children: ReactNode;
...
export function ResYandexAccountingReviewModal({
  reviewInfo,
  children,
  firmUrl,
  orderId,
  serviceId,
}: resYandexAccountingReviewModalProps) {
  ...
  const onRespAccountingReview = (data: any) => {
    toast('Добавление ответа на отзыв', {
      description: data.detail,
    });
    form.reset();
    queryClient.invalidateQueries(['getAccountingReviewsYandex']);
    setOpen(false);
  };

  const { mutate, isLoading } = useMutation(
    clients.addRespAccountingReviewYandex.fn,
    {
      onSuccess: (data) => onRespAccountingReview(data),
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    mutate({
      body: {
        ...
        ...cleanedData,
      } as addRespAccountingReviewYandexReq,
    });
    setOpen(false);
  }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='max-w-2xl p-7'>
        <DialogHeader className='mb-3'>
          <DialogTitle>Форма ответа на отзыв</DialogTitle>
        </DialogHeader>
        <div className='flex flex-col gap-2'>
          <p className='font-medium text-text-primary'>Url фирмы</p>
          <div className='bg-layer-02 px-2 py-1 rounded'>
            <p
              className={cn(
