...
import { addRespAccountingReviewGisReq } from '@/api/gis/addRespAccountingReviewGis';

interface ResGisAccountingReviewModalProps {
...
export function ResGisAccountingReviewModal({
  reviewInfo,
  children,
  firmUrl,
  orderId,
  serviceId,
}: ResGisAccountingReviewModalProps) {
  ...
  const onRespAccountingReviewGis = (data: any) => {
    toast('Добавление ответа на отзыв', {
      description: data.detail,
    });
    form.reset();
    queryClient.invalidateQueries(['getAccountingReviewsGis']);
    setOpen(false);
  };

  const { mutate, isLoading } = useMutation(
    clients.addRespAccountingReviewGis.fn,
    {
      onSuccess: (data) => onRespAccountingReviewGis(data),
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    mutate({
      body: {
        ...
        ...cleanedData,
      } as addRespAccountingReviewGisReq,
    });
    setOpen(false);
  }
